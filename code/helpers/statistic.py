...
class Statistic(object):
  def __init__(self, sess, data, model_dir, variables, test_step, max_to_keep=20):
    ...
    #self.writer = tf.compat.v1.summary.FileWriter('./logs/%s' % self.model_dir, self.sess.graph) #for linux and mac
    ...
    with tf.compat.v1.variable_scope('summary'):
      ...
      for tag in scalar_summary_tags:
        ...
        self.summary_ops[tag]  = tf.compat.v1.summary.scalar('%s/%s' % (data, tag), self.summary_placeholders[tag])
